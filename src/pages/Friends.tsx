...
export default function Friends() {
  ...
  const steps = [
    {
      title: "添加本站友链",
      desc: "请先在您的网站友链页面添加本站信息，可直接复制下方内容",
    },
    {
      title: "提交申请",
      desc: (
  ...
  return (
    <>
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        ...
        <Badge
          variant="secondary"
          className="gap-1.5 rounded-full px-3 py-1 text-[12.5px]"
        >
      ...
      <div className="mb-4 grid gap-4 md:grid-cols-2">
        ...
        <Card className="p-6">
          ...
          {infoFields.map(({ key, label, icon: Ico }) => (
            <div
              key={key}
              className="mb-2 flex items-center justify-between gap-2 rounded-[9px] border border-border/60 bg-muted/50 px-3 py-2"
            >
          ...
          <div className="mt-3 flex items-center justify-between gap-2 rounded-[9px] border border-primary/25 bg-primary/5 px-3 py-2.5">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-[11.5px] text-muted-foreground">申请邮箱</p>
                <p className="min-w-0 text-[12.8px] font-medium [overflow-wrap:anywhere] [word-break:break-all]">
                  <a
                    href={"mailto:" + siteInfo.email}
        ...
        <Card className="flex flex-col p-6">
          ...
          <ol className="flex-1 space-y-4">
            {steps.map((s, i) => (
              <li key={i} className="relative flex gap-3">
                ...
                <span className="z-10 flex size-[27px] shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary to-[hsl(260,80%,55%)] text-[12.5px] font-bold text-primary-foreground">
                  {i + 1}
                </span>
                <div className="min-w-0">
          ...
          <div className="mt-4 rounded-[10px] border border-border bg-muted/50">
            ...
            <pre className="overflow-x-auto whitespace-pre-wrap p-3 font-mono text-[12px] leading-relaxed text-muted-foreground [overflow-wrap:anywhere] [word-break:break-word]">
              {friendTemplate}
            </pre>
          </div>
        </Card>
      ...
      <Card className="mb-4 p-5">
        ...
        <div className="grid gap-x-6 gap-y-0.5 sm:grid-cols-2">
          {friendNotes.map((note) => (
            <div
              key={note.title}
              className="flex items-baseline gap-2.5 py-[7px] text-[13.5px] text-muted-foreground"
            >
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-4">
        {friends.map((f) => (
          <a
            key={f.title}
            className="group flex min-w-0 flex-col gap-3 rounded-xl border border-border/70 bg-card p-4 transition-all duration-200 hover:-translate-y-1 hover:border-primary/50 hover:shadow-lg"
            href={f.siteurl}
            target="_blank"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 line-clamp-2 text-[12px] leading-relaxed text-muted-foreground [overflow-wrap:anywhere] [word-break:break-word]">
                  {f.desc}
                </p>
              </div>
            </div>
